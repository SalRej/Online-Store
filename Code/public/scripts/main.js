...
if(typeof(products)!='undefined'){
    ...
    for(type of sizeTypes){
        ...
        let checkbox =$("<input>",{class:"test",type:"checkbox",id:type});
        $(".size-variants-holder").append(checkbox);
        $(".size-variants-holder").append(label);
    }
    ...
    $('.submit-filters').click(function(){
            ...
            if(chosenColors.length>0){
                for(imageGroup of product.image_groups){
                    if(imageGroup.view_type=="swatch"){
                        if(!chosenColors.includes(imageGroup.variation_value)){        
                            toShow=false;
                        }
                    }
                }
        ...
        $(".num-products").text(countProducts);
    })
}